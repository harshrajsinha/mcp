...
def create_default_scikiq_config() -> Dict[str, Any]:
    return {
...
def create_default_config_helper(dry_run: bool):
    config_path = Path(os.getcwd()) / "scikiq_config.yaml"
    config_data = create_default_scikiq_config()
    if dry_run:
        pp(config_data)
        return
...
@cc.command("scikiq", help="Create a default configuration file for SCIKIQ")
